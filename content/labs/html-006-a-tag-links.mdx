...
<HtmlCssDemo
  title="例1：押しやすいテキストリンク（色指定なし）"
  html={String.raw`<a class="link" href="/contact.html">お問い合わせ</a>`}
  css={String.raw`.link {
...
<HtmlCssDemo
  title="例2：ボタン“風”リンク（遷移はaでOK）"
  html={String.raw`<a class="cta" href="/signup.html">無料で始める</a>`}
  css={String.raw`.cta {
  display: inline-block;
  padding: 10px 14px;
...
<HtmlCssDemo
  title="例3：リスト型ナビ（複数リンクを整理）"
  html={String.raw`<nav class="nav" aria-label="サイト内メニュー">
